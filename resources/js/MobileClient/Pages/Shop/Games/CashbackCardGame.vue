    ...
    <div class="card-game-page">
        ...
        <div v-if="!gameAvailable" class="game-unavailable">
            <div class="unavailable-content">
                ...
                <h2 class="unavailable-title">Игра недоступна</h2>
                <p class="unavailable-text">Карточная игра временно отключена</p>
            </div>
        </div>
        ...
        <template v-else>
            ...
            <div class="game-content">
                ...
                <div class="rules-section">
                    ...
                    <transition name="slide-down">
                        <div v-if="showRules" class="rules-content">
                            <div class="rules-text" v-if="gameSettings.rules">
                                {{ gameSettings.rules }}
                            </div>
                ...
                <!-- ========================================== -->
                <!-- СЕТКА КАРТОЧЕК (динамический размер) -->
                <!-- ========================================== -->
...
export default {
    ...
    computed: {
        ...
        gridStyle() {
            return {
                gridTemplateColumns: `repeat(${this.gameSettings.grid_columns}, 1fr)`,
                maxWidth: `${this.gameSettings.grid_columns * 120}px`
            };
        },

        nextPlayText() {
            if (!this.lastPlayDate) return 'Сыграйте ещё раз';
            ...
            if (nextPlay <= now) return 'Можно играть снова!';

            const diffTime = nextPlay - now;
            const diffHours = Math.ceil(diffTime / (1000 * 60 * 60));
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

            if (diffDays > 1) return `Следующая игра через ${diffDays} дн.`;
            if (diffHours > 1) return `Следующая игра через ${diffHours} ч.`;
    ...
    methods: {
        ...
        // Дефолтные призы (если настройки не загрузились)
        getDefaultPrizes() {
            return [
                { id: 1, title: 'Монетка', description: 'Небольшой бонус', icon: 'fa-solid fa-coins', value: 10, rarity: 'common' },
                { id: 2, title: 'Кристалл', description: 'Редкий кристалл', icon: 'fa-solid fa-gem', value: 50, rarity: 'rare' },
                { id: 3, title: 'Корона', description: 'Королевский бонус', icon: 'fa-solid fa-crown', value: 200, rarity: 'epic' },
                { id: 4, title: 'Джекпот', description: 'ЛЕГЕНДАРНЫЙ ПРИЗ!', icon: 'fa-solid fa-trophy', value: 1000, rarity: 'legendary' }
            ];
        },
...
.card-front.rarity-common { border-color: #6c757d; }
.card-front.rarity-common .card-rarity-indicator { background: linear-gradient(90deg, #6c757d 0%, #adb5bd 100%); }
.card-front.rarity-common .card-prize-icon { background: linear-gradient(135deg, #6c757d 0%, #adb5bd 100%); color: white; }

.card-front.rarity-rare { border-color: #0d6efd; box-shadow: 0 0 20px rgba(13, 110, 253, 0.3); }
.card-front.rarity-rare .card-rarity-indicator { background: linear-gradient(90deg, #0d6efd 0%, #6610f2 100%); }
.card-front.rarity-rare .card-prize-icon { background: linear-gradient(135deg, #0d6efd 0%, #6610f2 100%); color: white; }

.card-front.rarity-epic { border-color: #6f42c1; box-shadow: 0 0 25px rgba(111, 66, 193, 0.4); }
.card-front.rarity-epic .card-rarity-indicator { background: linear-gradient(90deg, #6f42c1 0%, #d63384 100%); }
.card-front.rarity-epic .card-prize-icon { background: linear-gradient(135deg, #6f42c1 0%, #d63384 100%); color: white; animation: epicPulse 2s ease-in-out infinite; }
...
.card-front.rarity-legendary { border-color: #ffc107; box-shadow: 0 0 30px rgba(255, 193, 7, 0.5); background: linear-gradient(135deg, rgba(255, 215, 0, 0.1) 0%, rgba(255, 152, 0, 0.05) 100%); }
.card-front.rarity-legendary .card-rarity-indicator { background: linear-gradient(90deg, #ffc107 0%, #ff9800 50%, #ff5722 100%); height: 6px; }
.card-front.rarity-legendary .card-prize-icon { background: linear-gradient(135deg, #ffd700 0%, #ff9800 100%); color: #1a1a1a; animation: legendaryGlow 1.5s ease-in-out infinite; }
...
.result-rarity-badge.rarity-legendary { background: linear-gradient(135deg, rgba(255, 215, 0, 0.2) 0%, rgba(255, 152, 0, 0.2) 100%); color: #b8860b; border: 1px solid rgba(255, 215, 0, 0.3); }
...
.prize-icon.rarity-legendary { background: linear-gradient(135deg, #ffd700 0%, #ff9800 100%); color: #1a1a1a; }
