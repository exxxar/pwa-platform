    ...
    <div class="dialog-list">
        ...
        <div class="tabs-wrapper">
            <button class="tab-btn" :class="{ active: activeTab === 'all' }" @click="activeTab = 'all'">
                ...
                <span>Все</span>
                <span v-if="activeDialogsCount > 0" class="tab-badge">{{ activeDialogsCount }}</span>
            </button>
            <button class="tab-btn" :class="{ active: activeTab === 'archived' }" @click="activeTab = 'archived'">
                <i class="fa-solid fa-box-archive"></i>
                <span>Архив</span>
                <span v-if="archivedDialogsCount > 0" class="tab-badge muted">{{ archivedDialogsCount }}</span>
            </button>
            <button v-if="!isEditMode && currentTabDialogs.length > 0" class="select-mode-btn" @click="enterEditMode">
        ...
        <transition name="slide-down">
            <div v-if="isEditMode" class="selection-panel">
                <div class="selection-left">
                    ...
                    <span class="selection-count">Выбрано: <strong>{{ selectedDialogs.length }}</strong></span>
                </div>
                <div class="selection-actions">
                    <button v-if="activeTab === 'all'" class="action-btn archive" @click="archiveSelected" :disabled="selectedDialogs.length === 0" title="Архивировать">
                        <i class="fa-solid fa-box-archive"></i>
                    </button>
                    <button v-if="activeTab === 'archived'" class="action-btn restore" @click="restoreSelected" :disabled="selectedDialogs.length === 0" title="Восстановить">
                        <i class="fa-solid fa-rotate-left"></i>
                    </button>
                    <button class="action-btn delete" @click="deleteSelected" :disabled="selectedDialogs.length === 0" title="Удалить навсегда">
                        <i class="fa-solid fa-trash"></i>
                    </button>
                    <button class="action-btn cancel" @click="exitEditMode" title="Отмена">
        ...
        <div v-if="activeTab === 'archived' && archivedDialogsCount > 0" class="archive-actions">
            <button class="archive-action-btn" @click="emptyArchiveConfirm">
                <i class="fa-solid fa-trash-can"></i> Очистить весь архив
            </button>
        </div>
        ...
        <div class="search-wrapper">
            <div class="search-box">
                ...
                <input v-model="searchQuery" type="text" :placeholder="activeTab === 'all' ? 'Поиск по чатам...' : 'Поиск в архиве...'" class="search-input">
        ...
        <div v-else class="dialogs-container">
            <div v-for="group in groupedDialogs" :key="group.key" class="dialog-group">
                ...
                >
                    <label v-if="isEditMode" class="dialog-checkbox" @click.stop>
                        <input type="checkbox" :checked="isSelected(dialog)" @change="toggleSelect(dialog)">
                        <span class="checkbox-visual"><i class="fa-solid fa-check"></i></span>
                    </label>

                    <div class="dialog-avatar" :style="getAvatarStyle(dialog)">
                        <img v-if="getInterlocutor(dialog)?.avatar" :src="getInterlocutor(dialog).avatar" :alt="getInterlocutor(dialog)?.name">
                        <span v-else class="avatar-initials">{{ getInitials(getInterlocutor(dialog)?.name) }}</span>
                        <div v-if="getInterlocutor(dialog)?.is_online && activeTab === 'all'" class="online-dot"></div>
                        <div v-if="dialog.unread_count > 0 && activeTab === 'all'" class="unread-badge">
                            {{ dialog.unread_count > 99 ? '99+' : dialog.unread_count }}
                        </div>
                        <div v-if="dialog.is_pinned && activeTab === 'all'" class="pin-icon"><i class="fa-solid fa-thumbtack"></i></div>
                        <div v-if="dialog.is_archived" class="archive-icon"><i class="fa-solid fa-box-archive"></i></div>
                    </div>

                    <div class="dialog-info">
                        <div class="dialog-header">
                            <span class="dialog-name">{{ dialog?.title || 'Без имени' }}</span>
                            <span class="dialog-time">{{ formatDialogTime(dialog.last_message_at) }}</span>
                        </div>
                        <div class="dialog-preview">
                            <i v-if="isMyLastMessage(dialog) && activeTab === 'all'" class="message-status" :class="getMessageStatusClass(dialog)"></i>
                            <span class="preview-text">{{ getLastMessagePreview(dialog.last_message) }}</span>
                        </div>
                    </div>

                    <button v-if="!isEditMode" class="dialog-menu-btn" @click.stop="showContextMenu($event, dialog)">
                    ...
                    <div class="context-menu" :style="{ top: contextMenu.y + 'px', left: contextMenu.x + 'px' }" @click.stop>
                        <div class="context-menu-header">
                            <div class="context-menu-avatar" :style="getAvatarStyle(contextMenu.dialog)">
                                <img v-if="getInterlocutor(contextMenu.dialog)?.avatar" :src="getInterlocutor(contextMenu.dialog).avatar">
                                <span v-else class="avatar-initials">{{ getInitials(getInterlocutor(contextMenu.dialog)?.name) }}</span>
                            </div>
                        ...
                        <button class="context-menu-item" @click="viewAttachments(contextMenu.dialog)">
                            <i class="fa-solid fa-paperclip"></i><span>Вложения</span>
                            <span v-if="contextMenu.attachmentsCount" class="menu-badge">{{ contextMenu.attachmentsCount }}</span>
                        </button>
                        <button v-if="activeTab === 'all'" class="context-menu-item" @click="togglePin(contextMenu.dialog)">
                        ...
                        <button v-if="activeTab === 'all'" class="context-menu-item" @click="markAsRead(contextMenu.dialog)">
                            <i class="fa-solid fa-check-double"></i><span>Прочитано</span>
                        </button>
                        <div class="context-menu-divider"></div>
                        <button v-if="activeTab === 'all'" class="context-menu-item archive" @click="archiveSingle(contextMenu.dialog)">
                            <i class="fa-solid fa-box-archive"></i><span>В архив</span>
                        </button>
                        <button v-if="activeTab === 'archived'" class="context-menu-item restore" @click="restoreSingle(contextMenu.dialog)">
                        ...
                        <button class="context-menu-item delete" @click="deleteSingle(contextMenu.dialog)">
                            <i class="fa-solid fa-trash"></i><span>Удалить навсегда</span>
                        </button>
                    </div>
                </div>
            </transition>
                ...
                <div v-if="showAttachmentsModal" class="modal-overlay" @click.self="showAttachmentsModal = false">
                    <div class="attachments-modal">
                        <div class="modal-header">
                            ...
                            <button class="modal-close" @click="showAttachmentsModal = false"><i class="fa-solid fa-xmark"></i></button>
                        </div>
                        ...
                        <div v-else-if="attachments.length === 0" class="attachments-empty">
                            <i class="fa-solid fa-inbox"></i><p>Вложений нет</p>
                        </div>
...
export default {
    name: "DialogList", // Важно для keep-alive
    components: { SkeletonLoader },
    // emits: ['select-dialog'], // УДАЛЕНО
    ...
    data() {
        return {
            ...
            isEditMode: false,
            selectedDialogs: [],
            contextMenu: { visible: false, x: 0, y: 0, dialog: null, messagesCount: 0, attachmentsCount: 0 },
    ...
    computed: {
        ...
        activeDialogs() { return (this.sortedDialogs || []).filter(d => !d.is_archived); },
        archivedDialogs() { return (this.sortedDialogs || []).filter(d => d.is_archived); },
        activeDialogsCount() { return this.activeDialogs.length; },
        archivedDialogsCount() { return this.archivedDialogs.length; },
        currentTabDialogs() {
        ...
        groupedDialogs() {
            if (this.activeTab === 'archived') return [{ key: 'archived', label: '📦 Архив', dialogs: this.currentTabDialogs }];
            const groups = {
            ...
            this.currentTabDialogs.forEach(dialog => {
                if (dialog.is_pinned) { groups.pinned.dialogs.push(dialog); return; }
                const messageDate = new Date(dialog.last_message_at || dialog.updated_at || 0);
                const messageDay = new Date(messageDate.getFullYear(), messageDate.getMonth(), messageDate.getDate());
                if (messageDay.getTime() === today.getTime()) groups.today.dialogs.push(dialog);
                else if (messageDay.getTime() === yesterday.getTime()) groups.yesterday.dialogs.push(dialog);
                else if (messageDay > weekAgo) groups.week.dialogs.push(dialog);
                else groups.older.dialogs.push(dialog);
            });
            return Object.values(groups).filter(group => group.dialogs.length > 0).sort((a, b) => a.order - b.order);
        },
        isAllSelected() { return this.currentTabDialogs.length > 0 && this.selectedDialogs.length === this.currentTabDialogs.length; },
        isIndeterminate() { return this.selectedDialogs.length > 0 && this.selectedDialogs.length < this.currentTabDialogs.length; },
    },

    watch: {
        activeTab() { this.exitEditMode(); },
    },

    async mounted() {
        if (!this.isHydrated) await this.loadDialogs();
    },
    ...
    methods: {
        isActive(dialog) { return this.currentDialog?.id === dialog.id; },

        async open(dialog) {
            if (this.isActive(dialog)) return;
            // 🚀 ПЕРЕХОД ЧЕРЕЗ ROUTER
            this.router.push({ name: 'ChatRoom', params: { id: dialog.id } });
        },

        handleDialogClick(dialog) {
            if (this.isEditMode) this.toggleSelect(dialog);
            else this.open(dialog);
        },

        enterEditMode() { this.isEditMode = true; this.selectedDialogs = []; },
        exitEditMode() { this.isEditMode = false; this.selectedDialogs = []; },
        isSelected(dialog) { return this.selectedDialogs.includes(dialog.id); },
        toggleSelect(dialog) {
            const index = this.selectedDialogs.indexOf(dialog.id);
            if (index === -1) this.selectedDialogs.push(dialog.id);
            else this.selectedDialogs.splice(index, 1);
        },
        toggleSelectAll() {
            if (this.isAllSelected) this.selectedDialogs = [];
            else this.selectedDialogs = this.currentTabDialogs.map(d => d.id);
        },

        // ... (Методы archiveSelected, restoreSelected, deleteSelected, archiveSingle, restoreSingle, deleteSingle, togglePin, markAsRead, emptyArchiveConfirm остаются БЕЗ ИЗМЕНЕНИЙ, как в вашем коде) ...
        // Для экономии места я их сократил, но вставьте сюда ваши оригинальные методы из вашего кода. Они работают идеально.
        ...
        showContextMenu(event, dialog) {
            ...
            const menuWidth = 280, menuHeight = 400;
            if (x + menuWidth > window.innerWidth) x = window.innerWidth - menuWidth - 10;
            if (y + menuHeight > window.innerHeight) y = window.innerHeight - menuHeight - 10;
            this.contextMenu = { visible: true, x, y, dialog, messagesCount: dialog.messages_count || 0, attachmentsCount: dialog.attachments_count || 0 };
        },
        closeContextMenu() { this.contextMenu.visible = false; this.contextMenu.dialog = null; },
        ...
        getInterlocutor(dialog) { return getInterlocutor(dialog); },
        ...
        formatDialogTime(timestamp) { return formatDialogTime(timestamp); },
        getLastMessagePreview(message) { return getLastMessagePreview(message); },
        isMyLastMessage(dialog) {
            if (!dialog.last_message || !this.self) return false;
            return dialog.last_message.is_mine === true || dialog.last_message.meta?.user_id === this.self.id;
        },
