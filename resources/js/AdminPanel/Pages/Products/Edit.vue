    ...
    <div class="product-edit-page">
        <div class="page-header">
            <div>
                ...
                <p class="page-subtitle">{{ product?.name || 'Загрузка...' }}</p>
            </div>
        </div>
        ...
        <form v-else-if="product" @submit.prevent="handleSubmit" class="product-form">
            <!-- Main Info -->
...
const loadProduct = async () => {
    ...
    try {
        ...
        Object.keys(form).forEach(key => {
            if (key === 'category_ids') {
                form[key] = product.value.categories?.map(c => c.id) || []
            } else if (key === 'images') {
                form[key] = product.value.images || []
            } else if (product.value[key] !== undefined) {
                form[key] = product.value[key]
            }
        })
    } catch (error) {
        notifications.error('Ошибка при загрузке товара')
        console.error(error)
    } finally {
        loading.value = false
    }
...
const handleSubmit = async () => {
    ...
    try {
        ...
        notifications.success('Товар успешно обновлен')
        ...
        if (error.response?.status === 422) {
            ...
            notifications.error('Ошибка при обновлении товара')
        }
    } finally {
        saving.value = false
    }
